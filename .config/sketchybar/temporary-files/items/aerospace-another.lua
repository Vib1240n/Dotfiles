...
local generate_app_icons_string = function(space_id)
	local visible_apps_command = io.popen("aerospace list-windows --format %{app-name} --workspace " .. space_id)
		:read("*a")
...
local generate_app_icons_table = function(space_id)
	local visible_apps =
		utils.json.decode(io.popen("aerospace list-windows --json --workspace " .. space_id):read("*a"))
