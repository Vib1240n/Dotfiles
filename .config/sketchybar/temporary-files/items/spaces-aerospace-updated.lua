...
local LIST_ALL_WORKSPACES = "aerospace list-workspaces --all"
...
local function updateSpaceIcons(spaceId, workspaceName)
	...
	sbar.exec(LIST_APPS:format(workspaceName), function(appsOutput)
		...
		if not appFound then
			icon_strip = " - "
			shouldDraw = false -- Changed to false for empty workspaces
		end
...
local function addWorkspaceItem(workspaceName, monitorId, isSelected)
	...
	if not spaces[spaceId] then
		local space_item = sbar.add("item", spaceId, {
			...
			display = monitorId,
			drawing = true, -- Initially visible
		})
	...
	spaces[spaceId].item:set({
		...
		drawing = true, -- Make sure it's visible when it has content
	})
	...
	spaces[spaceId].bracket:set({
		...
		drawing = true, -- Make sure bracket is visible
	})
...
local function removeWorkspaceItem(spaceId)
	if spaces[spaceId] then
		-- Hide the items instead of removing them completely
		spaces[spaceId].item:set({ drawing = false })
		spaces[spaceId].bracket:set({ drawing = false })

		-- Alternatively, you can completely remove them:
		-- sbar.exec("sketchybar --remove " .. spaceId)
		-- spaces[spaceId] = nil
	end
end
...
local function drawSpaces()
	sbar.exec(LIST_MONITORS, function(monitorsOutput)
		...
		sbar.exec(LIST_CURRENT, function(focusedWorkspaceOutput)
			...
			-- Get all workspaces that should be visible (non-empty)
			local activeWorkspaces = {}

			-- Iterate through monitors and get non-empty workspaces
			for monitorId in monitorsOutput:gmatch("[^\r\n]+") do
				sbar.exec(LIST_WORKSPACES, function(workspacesOutput)
					for workspaceName in workspacesOutput:gmatch("[^\r\n]+") do
						if workspaceName and workspaceName ~= "" then
							activeWorkspaces[workspaceName] = monitorId
							local isSelected = workspaceName == focusedWorkspace
							addWorkspaceItem(workspaceName, monitorId, isSelected)
						end
					end

					-- After processing all active workspaces, hide any that are no longer active
					sbar.exec(LIST_ALL_WORKSPACES, function(allWorkspacesOutput)
						for workspaceName in allWorkspacesOutput:gmatch("[^\r\n]+") do
							if workspaceName and workspaceName ~= "" then
								local spaceId = "workspace_" .. workspaceName
								-- If this workspace is not in our active list and exists in spaces, hide it
								if not activeWorkspaces[workspaceName] and spaces[spaceId] then
